...
class GreedyLocalSearchHeuristic(PostHocHeuristic):
    ...
    def __init__(
        ...
        self.operators_max_constraint_value = {
            operator: max(
    ...
    def _compute_operator_performance(self, operator: str) -> float:
        return len(self.operators_constraints[operator]) / (
            self.operators_costs[operator]
            * ceil(
                self.operators_max_constraint_value[operator]
                / self.operators_costs[operator]
            )
        )

    def _compute_tie_breaking(self, a: str, b: str) -> str:
        if self.tie_breaking_operation == TieBreakingOperation.MAX_CONSTRAINT:
            return self._compute_tie_breaking_max_constraint(a, b)
        elif self.tie_breaking_operation == TieBreakingOperation.SUM_CONSTRAINTS:
            return self._compute_tie_breaking_sum_constraints(a, b)
        elif self.tie_breaking_operation == TieBreakingOperation.SUM_SQUARE_CONSTRAINTS:
            return self._compute_tie_breaking_sum_square_constraints(a, b)

        raise Exception("TieBreakingOperation not found")

    def _compute_tie_breaking_max_constraint(self, a: str, b: str) -> str:
        return self.__compute_tie_breaking_lambda_for_operators(a, b, max)

    def _compute_tie_breaking_sum_constraints(self, a: str, b: str) -> str:
        return self.__compute_tie_breaking_lambda_for_operators(a, b, sum)

    def _compute_tie_breaking_sum_square_constraints(self, a: str, b: str) -> str:
    ...
    def __get_best_operator(self) -> str:
        ...
        for operator in self.operators:
            operator_performance = self._compute_operator_performance(operator)
            ...
            if best_operator_performance == operator_performance:
                best_operator = self._compute_tie_breaking(best_operator, operator)
    ...
    def _compute_post_hoc(self):
        ...
        while any([value > 0 for value in self.constraints_values.values()]):
            ...
            times_to_increment = self._compute_times_to_increment(best_operator)
